...
theta = torch.tensor([4.], device="cuda")
...
for n in tqdm(range(nmax)):
    if n==0:
        ...
        pfor_list_n_prev = pfor_list[n-1]
        Z_list_n_prev = Z_list[n-1]
        A = A

    pfor_list[n], Z_list[n], A = propagate_pressure(pfor_list_n_prev=pfor_list_n_prev, 
                                                    Z_list_n_prev=Z_list_n_prev, 
                                                    A=A,
                                                    A0=A0,
                                                    n=n,
                                                    a_list_n=a_list[n], # starting from here, all the args below are preset or constant
                                                    rho_list_n=rho_list[n], 
                                                    c_list_n=c_list[n], 
                                                    fMHz=fMHz, 
                                                    f=f, 
                                                    mmax=mmax, 
...
for t in tqdm(range(pfor.shape[-1]-1, 0, -1)):
    f_t = pfor_list[t] - pfor_list[t-1]
    lamda = lamda_list[..., t]
    vjp_p = torch.autograd.grad(f_t, pfor_list[t], grad_outputs=lamda, retain_graph=True)
    lamda_list[..., t-1] = lamda_list[..., t]+vjp_p[0]
    vjp_theta = torch.autograd.grad(f_t, theta, grad_outputs=lamda, retain_graph=True)
    total_gradient += -vjp_theta[0]
    print(vjp_theta[0].item())

print(theta.detach().item()-total_gradient.detach().item())
